...
/**
 * builds a subgraph for chainstack to be deployed. 
 ...
 */
const buildChainstackSubgraph = (network: string, contractAddress: string, blockNumber: number, subgraphTemplate: string) => {
...
const main = async () => {
  ...
  if (_isSkip) {
    console.log("You are building the subgraph without any check, subgraph syncing may fail.".yellow);
    if (options.blockNumber == null) throw new Error("missing --blocknumber option".cyan)
    buildChainstackSubgraph(_network, _contractAddress, _blockNumber, _subgraphTemplate);
    return;
  }
  ...
  if (registryResult != 0) {
    buildChainstackSubgraph(_network, _contractAddress, registryResult, _subgraphTemplate);
    return;
  } else {
  ...
  if (!explorerResult) {
    ...
    console.log("If you still want to build subgraph add --skipCheck option".cyan);
  } else {
    buildChainstackSubgraph(_network, _contractAddress, _blockNumber, _subgraphTemplate);
  }
};
